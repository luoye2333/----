...
# ObstacleMap=np.ones((int(map_size_x/accuracy),
#                      int(map_size_y/accuracy)))
ObstacleMap=cv2.imread('map1.png',cv2.IMREAD_GRAYSCALE)
# cv2.imshow('123',ObstacleMap)
ObstacleMap[ObstacleMap == 0] = 1
ObstacleMap[ObstacleMap == 255] = 0
ObstacleMap=np.flipud(ObstacleMap)#图像是上到下，坐标系是下到上，所以要颠倒一下
fig,axes = plt.subplots(1,1)
fig.set_size_inches([7,7]) # 设置图像大小
# axes.imshow(ObstacleMap,cmap="binary",origin="upper")
# plt.pause(0.01)
...
while True:
    ...
    for i in range(len(ant_series)):
        ...
        m_ant.walk()
        ant_x.append(m_ant.current_pos_x)
        ant_y.append(m_ant.current_pos_y)
        if m_ant.isEnd():
    ...
    axes.scatter(ant_x, ant_y, color="red",s=1)
    # axes.imshow(np.flipud(ObstacleMap),cmap="binary")
    axes.imshow(ObstacleMap,cmap="binary")
